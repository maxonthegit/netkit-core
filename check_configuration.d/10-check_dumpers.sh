...
SYSTEM_COMMANDS="wireshark tcpdump"
NETKIT_UTILITIES="uml_dump"
...
if [ $OK = 0 ]; then
   echo "failed!"
   echo
   echo "*** Warning: some of the standard tools needed for running Netkit were"
...
if [ "$PATH_TO_BE_USED" != ":" ]; then
   echo "failed!"
   echo
   echo "*** Warning: all the standard tools required by Netkit have been found on"
